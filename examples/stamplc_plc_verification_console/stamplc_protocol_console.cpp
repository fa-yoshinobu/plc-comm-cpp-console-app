...
#ifndef PLC_CONSOLE_SLMP_HAS_TRAFFIC_STATS
#define PLC_CONSOLE_SLMP_HAS_TRAFFIC_STATS 0
#endif

#ifndef PLC_CONSOLE_BUILD_ENV
#define PLC_CONSOLE_BUILD_ENV "unavailable"
#endif

#ifndef PLC_CONSOLE_FIRMWARE_BUILD_ID
#define PLC_CONSOLE_FIRMWARE_BUILD_ID "unavailable"
#endif

#ifndef PLC_CONSOLE_PIO_CORE_VERSION
#define PLC_CONSOLE_PIO_CORE_VERSION "unavailable"
#endif

#ifndef PLC_CONSOLE_APP_SOURCE_ID
#define PLC_CONSOLE_APP_SOURCE_ID "unavailable"
#endif

#ifndef PLC_CONSOLE_SLMP_SOURCE_ID
#define PLC_CONSOLE_SLMP_SOURCE_ID "unavailable"
#endif

#ifndef PLC_CONSOLE_MC_SOURCE_ID
#define PLC_CONSOLE_MC_SOURCE_ID "unavailable"
#endif

#ifndef PLC_CONSOLE_UI_SOURCE_ID
...
char g_slmp_host[48] = {};
uint16_t g_slmp_tcp_port = plc_console_board::kDefaultSlmpTcpPort;
uint16_t g_slmp_udp_port = plc_console_board::kDefaultSlmpUdpPort;
...
HardwareSerial* g_mc_uart = nullptr;
...
bool g_mc_hardware_rs485_mode_ready = false;
...
bool parseGpio(const char* text, int& value) {
    ...
    return plc_console_board::isExternalGpioAllowed(value);
}
...
const char* slmpProfileName(slmp::PlcProfile profile) {
    switch (profile) {
        ...
        case slmp::PlcProfile::MxR:
            return "melsec:mx-r";
#if PLC_CONSOLE_SLMP_HAS_MXR_RJ71_PROFILE
...
#if PLC_CONSOLE_SLMP_HAS_TRAFFIC_STATS
...
bool printEffectiveMcUart() {
    ...
    const bool expects_hardware_rs485 =
        g_mc_settings.interface_kind == McInterface::Rs485 &&
        plc_console_board::kUseHardwareRs485HalfDuplex;
    const bool mode_matches =
        !expects_hardware_rs485 || g_mc_hardware_rs485_mode_ready;
    const bool matches_requested =
        ...
        flow_control == expected_flow_control && mode_matches;
    ...
    Serial.print(F(" rs485_direction="));
    if (expects_hardware_rs485) {
        Serial.print(g_mc_hardware_rs485_mode_ready ? F("hardware-half-duplex") : F("not-configured"));
    } else if (g_mc_settings.interface_kind == McInterface::Rs485) {
        Serial.print(F("manual-hooks"));
    } else {
        Serial.print(F("not-applicable"));
    }
...
void mcRs485TxBegin(void*) {
    digitalWrite(plc_console_board::kRs485DirectionPin, plc_console_board::kRs485TxEnable);
    delayMicroseconds(100U);
}

void mcRs485TxEnd(void*) {
    digitalWrite(plc_console_board::kRs485DirectionPin, plc_console_board::kRs485RxEnable);
}

void setMcRs485ReceiveMode() {
    if (!plc_console_board::kUseHardwareRs485HalfDuplex) {
        digitalWrite(plc_console_board::kRs485DirectionPin, plc_console_board::kRs485RxEnable);
    }
}
...
bool applyMcUart() {
    ...
    if (g_mc_settings.interface_kind == McInterface::Rs232 &&
        !plc_console_board::kHasOnboardRs232) {
        Serial.println(F("mc uart change rejected: this board has no onboard RS-232 interface"));
        return false;
    }
    if (g_mc_settings.interface_kind == McInterface::ExternalRtsCts &&
        !plc_console_board::kHasExternalRtsCts) {
        Serial.println(F("mc uart change rejected: this board has no approved external RTS/CTS interface"));
        return false;
    }

    g_mc_uart_ready = false;
    g_mc_hardware_rs485_mode_ready = false;
    if (g_mc_uart != nullptr) {
        g_mc_uart->end();
    }
    int rx_pin = plc_console_board::kRs232RxPin;
    int tx_pin = plc_console_board::kRs232TxPin;
    if (g_mc_settings.interface_kind == McInterface::Rs485) {
        g_mc_uart = &Serial1;
        g_mc_uart_port = UART_NUM_1;
        rx_pin = plc_console_board::kRs485RxPin;
        tx_pin = plc_console_board::kRs485TxPin;
        if (!plc_console_board::kUseHardwareRs485HalfDuplex) {
            pinMode(plc_console_board::kRs485DirectionPin, OUTPUT);
            setMcRs485ReceiveMode();
        }
    } else {
        g_mc_uart = &Serial0;
    ...
    if (g_mc_settings.interface_kind == McInterface::Rs485 &&
        ...
        uart_mode_ok =
            uart_set_pin(
                ...
                tx_pin,
                rx_pin,
                plc_console_board::kRs485DirectionPin,
                UART_PIN_NO_CHANGE) == ESP_OK &&
            uart_set_mode(g_mc_uart_port, UART_MODE_RS485_HALF_DUPLEX) == ESP_OK;
        g_mc_hardware_rs485_mode_ready = uart_mode_ok;
    } else {
        uart_mode_ok = uart_set_mode(g_mc_uart_port, UART_MODE_UART) == ESP_OK;
    }
    delay(20U);
    drainMcUart();
    ...
    g_mc_uart_ready =
        buffers_ok && flow_ok && uart_mode_ok && protocol_ok && effective_ok;
    if (!g_mc_uart_ready) {
        g_mc_uart->end();
        setMcRs485ReceiveMode();
...
void printBoardStatus() {
    Serial.print(F("board_model="));
    Serial.print(plc_console_board::kModelName);
    Serial.print(F(" chip="));
...
void printHelp() {
    Serial.print(F("=== "));
    Serial.print(plc_console_board::kDisplayName);
    Serial.println(F(" PLC C++ verification console (read-only) ==="));
    ...
    if (plc_console_board::kHasOnboardRs232) {
        ...
        Serial.println(F("mc interface <rs485>"));
    }
    if (plc_console_board::kHasExternalRtsCts) {
...
void applyMcPreset(const char* name) {
    if (equalsIgnoreCase(name, "iqr-rs232")) {
        if (!plc_console_board::kHasOnboardRs232) {
            Serial.println(F("mc preset rejected: this board has no onboard RS-232 interface"));
            return;
        }
...
void handleMcCommand(char* tokens[], size_t count) {
    ...
    if (equalsIgnoreCase(tokens[1], "profile")) {
        ...
        if (count == 3U && equalsIgnoreCase(tokens[2], "rs232")) {
            if (!plc_console_board::kHasOnboardRs232) {
                Serial.println(F("mc interface rejected: this board has no onboard RS-232 interface"));
                return;
            }
            g_mc_settings.interface_kind = McInterface::Rs232;
            ...
            if (!plc_console_board::kHasExternalRtsCts) {
                Serial.println(F("mc interface rejected: this board has no approved external RTS/CTS interface"));
                return;
            }
            int rx = 0;
            int tx = 0;
            ...
            if (plc_console_board::kHasOnboardRs232) {
                ...
                Serial.println(F("mc interface usage: mc interface rs485"));
            }
            return;
        }
        (void)applyMcUart();
        ...
        if (count != 3U) {
            if (plc_console_board::kHasOnboardRs232) {
                ...
                Serial.println(F("mc preset usage: mc preset <iqr-rs485|q-rs485>"));
            }
            return;
        }
...
void setupConsole() {
    Serial.begin(plc_console_board::kUsbConsoleBaud);
    const uint32_t started = millis();
    while (!Serial && static_cast<uint32_t>(millis() - started) < 2500U) {
        delay(10U);
    }
    strncpy(g_slmp_host, plc_console_board::kDefaultSlmpHost, sizeof(g_slmp_host) - 1U);
    g_slmp_host[sizeof(g_slmp_host) - 1U] = '\0';
    ...
    WiFi.mode(WIFI_OFF);
    if (plc_console_board::kKeyPin >= 0) {
        pinMode(plc_console_board::kKeyPin, INPUT_PULLUP);
    }
    if (!plc_console_board::kUseHardwareRs485HalfDuplex) {
        pinMode(plc_console_board::kRs485DirectionPin, OUTPUT);
        setMcRs485ReceiveMode();
    ...
    Serial.println();
    Serial.print(plc_console_board::kDisplayName);
    Serial.println(F(" PLC C++ verification console"));
    Serial.print(F("firmware_build_id="));
    Serial.print(F(PLC_CONSOLE_FIRMWARE_BUILD_ID));
    Serial.print(F(" build_env="));
    Serial.print(F(PLC_CONSOLE_BUILD_ENV));
    Serial.print(F(" pio_core="));
    Serial.println(F(PLC_CONSOLE_PIO_CORE_VERSION));
    Serial.print(F("source_app="));
    Serial.print(F(PLC_CONSOLE_APP_SOURCE_ID));
    Serial.print(F(" dependency_slmp="));
    Serial.print(F(PLC_CONSOLE_SLMP_SOURCE_ID));
    Serial.print(F(" dependency_mc="));
    Serial.println(F(PLC_CONSOLE_MC_SOURCE_ID));
    Serial.print(F("dependency_ui="));
    Serial.println(F(PLC_CONSOLE_UI_SOURCE_ID));
