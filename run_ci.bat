...
echo [1/6] Building Atom Matrix console...
"%PIO_EXE%" run -e m5stack-atom-console
if %errorlevel% neq 0 exit /b %errorlevel%

echo [2/6] Building W6300 console...
"%PIO_EXE%" run -e wiznet_6300_evb_pico2
if %errorlevel% neq 0 exit /b %errorlevel%

echo [3/6] Building T-RSS3 PLC verification console...
"%PIO_EXE%" run -e t-rss3-verification-console
if %errorlevel% neq 0 exit /b %errorlevel%

echo [4/6] Building M5Stack StamPLC verification console...
"%PIO_EXE%" run -e stamplc-verification-console
if %errorlevel% neq 0 exit /b %errorlevel%

echo [5/6] Analyzing T-RSS3 owned sources...
"%PIO_EXE%" check -e t-rss3-verification-console --severity medium --severity high --fail-on-defect medium --fail-on-defect high
if %errorlevel% neq 0 exit /b %errorlevel%

echo [6/6] Analyzing StamPLC owned sources...
"%PIO_EXE%" check -e stamplc-verification-console --severity medium --severity high --fail-on-defect medium --fail-on-defect high
if %errorlevel% neq 0 exit /b %errorlevel%
